...
async function seedArticles() {
    const count = await database_1.default.article.count();
    if (count > 0)
        return;
    const articles = [
        {
    ...
    await database_1.default.article.createMany({ data: articles });
...
async function getArticleById(id) {
    const article = await database_1.default.article.findUnique({ where: { id } });
    if (!article) {
