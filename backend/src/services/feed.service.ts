 ...
 */
export async function getDealFlowFeed(investorId: string) {
  // Get all submitted ideas that are either PUBLIC or NDA_REQUIRED
  const publicAndNdaIdeas = await prisma.idea.findMany({
    where: {
      status: 'SUBMITTED',
  ...
  const invitedIdeas = await prisma.idea.findMany({
    where: {
      status: 'SUBMITTED',
 ...
 */
export async function getIdeaForInvestor(ideaId: string, investorId: string) {
