...
exports.getInvestorAIReview = getInvestorAIReview;
const database_1 = __importDefault(require("../config/database"));
const groq_1 = __importDefault(require("../config/groq"));
const client_s3_1 = require("@aws-sdk/client-s3");
const s3_1 = require("../config/s3");
async function triggerAnalysis(ideaId, userId) {
    ...
    const existingReport = await database_1.default.aIReport.findFirst({
        where: {
            ideaId,
    ...
    const report = await database_1.default.aIReport.create({
        data: {
            ideaId,
            userId,
            status: 'PROCESSING',
    ...
    processAnalysis(report.id, idea).catch((err) => {
        ...
        database_1.default.aIReport.update({
            where: { id: report.id },
...
async function extractFileContent(fileUrl) {
    if (!fileUrl)
        return null;
    try {
...
async function processAnalysis(reportId, idea) {
    ...
    try {
        ...
        await database_1.default.aIReport.update({
            where: { id: reportId },
            data: {
                status: 'COMPLETE',
                viabilityScore,
...
function buildAnalysisPrompt(idea, files) {
    // Parse JSON fields safely
    let useOfFunds = idea.useOfFunds || 'Not specified';
    let teamMembers = 'Not specified';
    try {
        if (idea.useOfFunds)
            useOfFunds = JSON.parse(idea.useOfFunds);
    }
    catch { }
    try {
        if (idea.teamMembers)
            teamMembers = JSON.parse(idea.teamMembers);
    }
    catch { }
...
async function getReport(ideaId, userId) {
    const report = await database_1.default.aIReport.findFirst({
        where: { ideaId, userId },
...
async function getReportStatus(ideaId, userId) {
    const report = await database_1.default.aIReport.findFirst({
        where: { ideaId, userId },
        orderBy: { createdAt: 'desc' },
...
const investorReviewCache = new Map();
const CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour
async function getInvestorAIReview(ideaId) {
    // Check cache
    ...
    // Fetch idea
    const idea = await database_1.default.idea.findUnique({
        where: { id: ideaId },
    });
    if (!idea) {
        throw Object.assign(new Error('Idea not found'), { statusCode: 404 });
